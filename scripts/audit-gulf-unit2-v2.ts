...
const msaFirst=resolveCurriculumItem(buildMsaUnit2CurriculumUnit('v2').items[0],getDialectContent('msa')).lessonWords[0];assert.equal(msaFirst.arabic,'البيت');assert.equal(msaFirst.transliteration,'al-bayt');assert.notEqual(msaFirst.transliteration,resolved[0].lessonWords[0].transliteration);const egyptianFirst=getDialectContent('egyptian').missions['unit2:around_the_home'].lessonWords[1];assert.equal(egyptianFirst.arabic,'أوضة النوم');assert.notEqual(egyptianFirst.transliteration,resolved[0].lessonWords[1].transliteration);
